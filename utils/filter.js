...
export default class Filter {
  ...
  correctFilterGenreCount = genres => {
    if (genres.length > 1) {
      const convertedGenres = genres.reduce((accumulator, item, index) => {
        let edited;
        if (index === 0) {
          edited = `${accumulator}${item.value}%`;
          return accumulator;
        }
        edited = `${accumulator}2C${item.value}%`;
        return edited;
      }, '');
  ...
  getFilterString = currentPage => {
    ...
    return `${searchPath}${page > 1 ? `${template.page}${page}&` : ''}${
      template.title
    }&${template.genres}${this.correctFilterGenreCount(
      genres,
    )}&st=0&sort=&stype=1&name_method=cw&name=&author_method=cw&author=&artist_method=cw&artist=&type=&rating_method=eq&rating=&released_method=eq&released=`;
  };
}
